function [grad_map_avg,gammaf] = grad_map_realign( gammahat,recovStruct)
...
%by gammaf (with possible distinct channel mismatches).
%%
...
gammaf=zeros(18*n_grid_p,size(gammahat,2));
    ...
    if any(z_secondMoments(:)>0)
        ...
        if any(grad_map_avg_t(:)>0.1)% filtering pixels with low likelihood of blinking
            ...
            for ii=1:length(I)
                ...
                grad_map_avg_t_local=grad_map_avg_t(I(ii)-2:I(ii)+2,J(ii)-2:J(ii)+2);
...
%         indx_lin_yz=sub2ind(size(distance_xz),gy_yz,gx_yz);
        ...
        %y channel
        gammaf(indx_lin+12*n_grid_p,i)=br_xx_t(ll)*deltax_xx*10^-2;
        gammaf(indx_lin+13*n_grid_p,i)=br_xx_t(ll)*deltay_xx*10^-2;
        ...
        gammaf(indx_lin+9*n_grid_p,i)=br_yy_t(ll)*deltay_yy*10^-2;
         ...
         %y channel
          gammaf(indx_lin+14*n_grid_p,i)=br_yy_t(ll)*deltax_yy*10^-2;
        gammaf(indx_lin+15*n_grid_p,i)=br_yy_t(ll)*deltay_yy*10^-2;       
        ...
        %y channel
        gammaf(indx_lin+16*n_grid_p,i)=br_zz_t(ll)*deltax_zz*10^-2;
        gammaf(indx_lin+17*n_grid_p,i)=br_zz_t(ll)*deltay_zz*10^-2;
