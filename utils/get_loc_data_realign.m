function loc_data=get_loc_data_realign(gammaf,recovStruct)
...
for i=1:n_frames
    ...
    %molecular parameters related to XX basis (both channels)
    z_xx=[z(1:n_grid_p);z(6*n_grid_p+1:7*n_grid_p);z(7*n_grid_p+1:8*n_grid_p);...
       z(12*n_grid_p+1:13*n_grid_p);z(13*n_grid_p+1:14*n_grid_p) ];
    
    %molecular parameters related to YY basis (both channels)
    z_yy=[z(n_grid_p+1:2*n_grid_p);z(8*n_grid_p+1:9*n_grid_p);...
        z(9*n_grid_p+1:10*n_grid_p);z(14*n_grid_p+1:15*n_grid_p);z(15*n_grid_p+1:16*n_grid_p)];
    
    %molecular parameters related to ZZ basis (both channels)
    z_zz=[z(2*n_grid_p+1:3*n_grid_p);z(10*n_grid_p+1:11*n_grid_p);....
        z(11*n_grid_p+1:12*n_grid_p);z(16*n_grid_p+1:17*n_grid_p);z(17*n_grid_p+1:18*n_grid_p)];
    ...
    if any(z_secondMoments(:)>0)
        ...
        while k<=size(loc_t,1)
            ...
            % XX basis
            %-----------------------------------------------------
            imx_xx_ychannel=pm(1,1)+(z_xx(im+3*n_grid_p)/(eps+z_xx(im)))*10^2; %x position
            imy_xx_ychannel=pm(1,2)+(z_xx(im+4*n_grid_p)/(eps+z_xx(im)))*10^2;%y position
            
            % YY basis
            %-----------------------------------------------------            
            imx_yy_ychannel=pm(1,1)+(z_yy(im+3*n_grid_p)/(eps+z_yy(im)))*10^2; %x position
            imy_yy_ychannel=pm(1,2)+(z_yy(im+4*n_grid_p)/(eps+z_yy(im)))*10^2;%y position
            
            % ZZ basis
            %-----------------------------------------------------
            imx_zz_ychannel=pm(1,1)+(z_zz(im+3*n_grid_p)/(eps+z_zz(im)))*10^2; %x position
            imy_zz_ychannel=pm(1,2)+(z_zz(im+4*n_grid_p)/(eps+z_zz(im)))*10^2;%y position            
            
            % combine position estimates
            %-----------------------------------------------------
            
            imx_ychannel=((imx_xx_ychannel*br_m_xx)+(imx_yy_ychannel*br_m_yy)+(imx_zz_ychannel*br_m_zz))/(eps+br_m);
            imy_ychannel=((imy_xx_ychannel*br_m_xx)+(imy_yy_ychannel*br_m_yy)+(imy_zz_ychannel*br_m_zz))/(eps+br_m);
            
            
            %update localizaton data
            %----------------------------------------------------
            loc_data=[loc_data; i, imx,imy,imx_ychannel,imy_ychannel,br_m,secondM];
