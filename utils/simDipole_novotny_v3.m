function [imgy, imgx] = simDipole_novotny_v3(polar_para, position_para, img_para, pmask)
...
%outputs
%---->--
%imgy-- 			image of dipole emitter at y channel
%imgx-- 			image of dipole emitter at x channel
...
zh = 0; % ?
z2 = 0; % ?
n1 = img_para.n; %refractive index of microscope
n2 = n1; % sample refractive index
nh = n1; % thin layer
...
Ex_common = bsxfun(@times, PupilFilt.*(1 ./ sqrt(cos(theta1))).*exp(1i * kh * zh * cos(thetah)).* ...
    exp(1i * k2 * z2 * cos(theta2)), ...
    (bsxfun(@times, cos(phi), Ep) + bsxfun(@times, -sin(phi), Es)));

Ey_1 = exp(1i*k1*(bsxfun(@times, reshape(z, 1, 1, K), cos(theta1)))); %defocus term at pupil plane
Ex_1 = exp(1i*k1*(bsxfun(@times, reshape(z, 1, 1, K), cos(theta1)))); %defocus term at pupil plane

Ey_2 = exp(1i*k1*(bsxfun(@times, reshape(deltax, 1, 1, K), sin(theta1) .* cos(phi)))); %phase shift
Ex_2 = exp(1i*k1*(bsxfun(@times, reshape(deltax, 1, 1, K), sin(theta1) .* cos(phi)))); %phase shift

Ey_3 = exp(1i*k1*(bsxfun(@times, reshape(deltay, 1, 1, K), sin(theta1) .* sin(phi)))); %phase shift
Ex_3 = exp(1i*k1*(bsxfun(@times, reshape(deltay, 1, 1, K), sin(theta1) .* sin(phi)))); %phase shift
...
imgy = (fftshift(fft2(Ey .* repmat((pmask), 1, 1, K))));
imgx = (fftshift(fft2(Ex .* repmat((pmaskRot), 1, 1, K))));
